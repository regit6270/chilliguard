"""Recommendation endpoints for soil and disease advice."""
...
@bp.route('/disease-treatment', methods=['POST'])
def get_disease_treatment() -> Tuple[Response, int]:
    """
    ...
    try:
        ...
        treatment = disease_info.get(severity_key) or disease_info.get('severity_low')
        if not isinstance(treatment, Mapping):
            return jsonify({
                'status': 'error',
                'message': f'No treatment steps recorded for severity "{severity}".'
            }), 404
        treatment_dict = cast(Mapping[str, Any], treatment)
        response = {
            'disease': disease_info.get(
                f'disease_{language}',
                disease_info['disease']),
            'severity': severity,
            'treatment': treatment_dict['treatment'],
            'chemical_option': treatment_dict['chemical'],
            'organic_option': treatment_dict['organic'],
            'frequency': treatment_dict['frequency'],
...
def get_fertilizer_schedule() -> Tuple[Response, int]:
    ...
    try:
        ...
        schedule: List[Dict[str, Any]] = [{"stage": "At Sowing",
                     "stage_hi": "बुवाई के समय",
                     "day": 0,
                     "fertilizers": [{"name": "Urea",
                                      "dosage_per_ha": 25,
                                      "unit": "kg"},
        ...
        for stage in schedule:
            ...
            if not isinstance(fertilizers, list):
                continue
            for fertilizer in fertilizers:  # type: ignore[index]
                fertilizer['total_quantity'] = round(
                    fertilizer['dosage_per_ha'] * field_area, 2)
